...
#%%
gdf = gpd.read_file(dire+"/bolivia/bolivia.shp")
...
# %%
tabla_estados = comparar_estados(bolivia,gdf)
tabla_estados
# %%
...
bolivia = pd.merge(bolivia, tabla_estados, left_on='Estado_norm', right_on='estado_censo')
# %%

tabla_comparativa = comparar_ciudades(bolivia, gdf, tabla_estados)
tabla_comparativa
# %%
...
tabla_comparativa[tabla_comparativa.similitud < 90]
# %%
bolivia = pd.merge(bolivia, tabla_comparativa, left_on=['Estado_norm','Ciudad_norm'],right_on= ['Estado_censo','nombre_censo'])
bolivia
# %%
...
# %%
bolivia = pd.merge(bolivia[bolivia.similitud >= 90]
...
# %%
bolivia['Amazonas'] = bolivia._merge == 'both'
bolivia.drop('_merge', axis = 1, inplace = True)
# %%
bolivia.to_csv(dire_guardado+'/bolivia.csv')
# %%
